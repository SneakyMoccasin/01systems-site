...
import { evaluateEligibilityMtcV1 } from "./evaluateEligibilityMtcV1";
import {
...
import { layer1ExecutionIdentityMtcV1, layer1StateIdentityMtcV1 } from "./executionIdentityMtcV1";
...
function immutableState(state: MutableState): Layer1RuntimeStateMtcV1 {
  ...
  return freezeDeep({ ...copy, stateIdentity: layer1StateIdentityMtcV1(copy) }) as unknown as Layer1RuntimeStateMtcV1;
}
...
export function runLayer1MtcV1(scenario: PreparedScenarioMtcV1, contract: TwoLayerMtcContractV1): Layer1ExecutionOutcomeMtcV1 {
  ...
  for (let period = scenario.horizon.firstPeriod; period <= scenario.horizon.finalPeriod; period += 1) {
    ...
    const eligibilityState = immutableState(state);
    const eligibility = candidates.map((item) => evaluateEligibilityMtcV1(item, eligibilityState, contract));
    ...
    if (admissionConflicts.length) {
      const base = {
        ...
        initialStateIdentity: initial.stateIdentity, initialState: initial, history: freezeDeep(structuredClone(history)), status: "failed-unresolved" as const,
    ...
    history.push(freezeDeep({ period, priorStateIdentity: committed.stateIdentity, completions: boundary.completions, releases: boundary.releases, scheduledCandidates: candidates.map((item) => item.instanceId), eligibility, admissions, reservations, entitlementConsumptions: consumptions, lifecycleTransitions: transitions, resultingStateIdentity: next.stateIdentity, resultingState: next }) as PeriodRecordMtcV1);
    committed = next;
  }
  ...
  const base = { executionVersion: TWO_LAYER_MTC_EXECUTION_VERSION, scenarioIdentity: scenario.semanticIdentity, contractIdentity, initialStateIdentity: initial.stateIdentity, initialState: initial, history: freezeDeep(history), status: "completed-horizon" as const, terminalBoundary, terminalState };
  return freezeDeep({ ...base, executionIdentity: layer1ExecutionIdentityMtcV1(base as never) });
}
