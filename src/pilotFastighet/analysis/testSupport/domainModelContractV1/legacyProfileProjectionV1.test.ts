...
test("locks exact derived diagnostic totals per profile",()=>{const totals={"legacy-real-estate-v1":17,"legacy-municipal-v1":13,"legacy-consulting-v1":15} as const;for(const profileId of PROFILES)assert.equal(requireProjection(profileId).diagnostics.length,totals[profileId],profileId);});
