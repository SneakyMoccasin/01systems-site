...
const ENVELOPE_HASHES = { "legacy-real-estate-v1": "sha256:0bb3e149951d1b583b55692d1716d551073889c53c289d8819b7409e23275bac", "legacy-municipal-v1": "sha256:355c59644b9fc2e32e62c79f2d3fb055e80ce5a1cd36688db444488bf4d43747", "legacy-consulting-v1": "sha256:37bf236ad981898d310dad92e5e65e2f0f9b495007dc9ac09ff32b2ec3c68bac" } as const;
...
test("declares closed compatibility propagation execution, triggers and implicit-node materialization",()=>{const expectedExecution={algorithm:"ordered-monotone-raise-fixed-point-v1",sourceReadPolicy:"missing-source-does-not-trigger-v1",targetReadPolicy:"missing-target-uses-declared-default-v1",targetComparison:"propagated-rank-strictly-greater-v1",writeVisibility:"later-occurrences-same-iteration-v1",iterationPolicy:"repeat-from-start-until-no-raise-v1",eventPolicy:{emission:"on-target-level-change-v1",step:"iteration-plus-one-v1",delaySteps:1,duplicateSuppression:"no-change-no-event-v1"}};const expectedNode={sourceNodeId:"liquidityPressure",adapterLocalNodeId:"liquidity-pressure",initialLevel:"absent",initialScore:"absent",targetMissingDefaultLevelId:"low",materialization:"on-propagation-raise-v1",scoreMaterialization:"projected-level-anchor-after-propagation-v1",impacts:"none"};for(const profileId of PROFILES){const propagation=requireProjection(profileId).envelope.compatibility.propagation;assert.deepEqual(propagation.executionSemantics,expectedExecution);assert.deepEqual(propagation.implicitNode,profileId==="legacy-municipal-v1"?null:expectedNode);for(const edge of propagation.compatibilityOnlyEdges){assert.equal(edge.triggerPredicate,"source-level-in-set-v1");assert.deepEqual(edge.triggerLevelIds,["high","severe"]);assert.equal(edge.projectedPropagatedLevelId,"high");assert.equal("missingReadDefaultLevelId" in edge,false);assert.equal("hasScore" in edge,false);}}});
test("binds every RE/CO compatibility raise and implicit score to the projected high anchor without direct impacts",()=>{for(const profileId of ["legacy-real-estate-v1","legacy-consulting-v1"] as const){const envelope=requireProjection(profileId).envelope,propagation=envelope.compatibility.propagation,scale=envelope.projection.contract.semanticPayload.scales.find(value=>value.scaleId==="legacy-risk-scale-v1");assert.ok(scale);const high=scale.levels.find(value=>value.levelId==="high");assert.ok(high);assert.equal(high.anchor,2);assert.ok(propagation.compatibilityOnlyEdges.length>0);for(const edge of propagation.compatibilityOnlyEdges)assert.equal(edge.projectedPropagatedLevelId,"high");assert.ok(propagation.implicitNode);assert.equal(propagation.implicitNode.scoreMaterialization,"projected-level-anchor-after-propagation-v1");assert.equal(propagation.implicitNode.impacts,"none");}});
test("locks exact derived diagnostic totals per profile",()=>{const totals={"legacy-real-estate-v1":15,"legacy-municipal-v1":12,"legacy-consulting-v1":14} as const;for(const profileId of PROFILES)assert.equal(requireProjection(profileId).diagnostics.length,totals[profileId],profileId);});
...
test("locks native identifier hygiene and checked-in fixture bytes", () => {
  const fixtureHashes = { "legacy-real-estate-v1": "0479c98e7308ee9211ab80dcd7997814a2efa3479292c635528977e12eafabab", "legacy-municipal-v1": "5c2c0d7ca687ebea62918d5a699ea4ea35fe6ae4a6286277ee0a0e935c57203d", "legacy-consulting-v1": "c3be1e34315bba049202946643da6670b9b07044563b3d399af8efcebed78169" } as const;
