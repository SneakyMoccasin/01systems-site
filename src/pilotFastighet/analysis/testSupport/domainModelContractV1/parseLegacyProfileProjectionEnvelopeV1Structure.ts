...
function propagation(value:unknown,path:string,c:Context):void {const v=object(value,path,c,["edgeEvaluationOrder","sourceEvaluationOrder","compatibilityOnlyEdges"]);if(!v)return;literal(v.edgeEvaluationOrder,"legacy-source-and-target-insertion-order-v1",`${path}/edgeEvaluationOrder`,c);each(array(v.sourceEvaluationOrder,`${path}/sourceEvaluationOrder`,c,LIMITS.maxPropagationEdges),`${path}/sourceEvaluationOrder`,(x,p)=>orderEntry(x,p,c));each(array(v.compatibilityOnlyEdges,`${path}/compatibilityOnlyEdges`,c,LIMITS.maxPropagationEdges),`${path}/compatibilityOnlyEdges`,(x,p)=>compatibilityEdge(x,p,c));}
...
function compatibilityEdge(value:unknown,path:string,c:Context):void {const keys=["sourcePosition","edgePosition","occurrencePosition","sourceLegacySourceDriverId","sourceLegacyTargetDriverId","adapterLocalSourceDriverId","adapterLocalTargetDriverId","compatibilityEdgeId","sourcePropagatedLevelId","projectedPropagatedLevelId","implicitSourceNodeId","adapterLocalImplicitNodeId","missingReadDefaultLevelId","materializeOnRaise","hasScore","hasImpacts"] as const;const v=object(value,path,c,keys);if(!v)return;positions(v,path,c,["sourcePosition","edgePosition","occurrencePosition"]);plainString(v.sourceLegacySourceDriverId,`${path}/sourceLegacySourceDriverId`,c);plainString(v.sourceLegacyTargetDriverId,`${path}/sourceLegacyTargetDriverId`,c);for(const key of ["adapterLocalSourceDriverId","adapterLocalTargetDriverId","compatibilityEdgeId","projectedPropagatedLevelId"] as const)id(v[key],`${path}/${key}`,c);plainString(v.sourcePropagatedLevelId,`${path}/sourcePropagatedLevelId`,c);literal(v.implicitSourceNodeId,"liquidityPressure",`${path}/implicitSourceNodeId`,c);literal(v.adapterLocalImplicitNodeId,"liquidity-pressure",`${path}/adapterLocalImplicitNodeId`,c);literal(v.missingReadDefaultLevelId,"low",`${path}/missingReadDefaultLevelId`,c);literal(v.materializeOnRaise,true,`${path}/materializeOnRaise`,c);literal(v.hasScore,false,`${path}/hasScore`,c);literal(v.hasImpacts,false,`${path}/hasImpacts`,c);}
