...
test("sustain exclusion binding is exact for RE and CO while MU remains ineligible",()=>{for(const profile of ["legacy-real-estate-v1","legacy-consulting-v1"] as const){const disposition=makeValidEnvelope(profile).semantic.compatibility.sustainThresholdDisposition;assert.ok(disposition);assert.equal(disposition.contractDisposition,"excluded-no-authoritative-value");assert.equal(disposition.valueStatus,"absent-unconfigured");}assert.equal(makeValidEnvelope("legacy-municipal-v1").semantic.compatibility.sustainThresholdDisposition,null);expectSemantic(x=>{x.compatibility.sustainThresholdDisposition=null;},"compatibility-declaration-mismatch","/compatibility/sustainThresholdDisposition","legacy-consulting-v1");expectSemantic(x=>{x.compatibility.sustainThresholdDisposition=structuredClone(makeValidEnvelope("legacy-real-estate-v1").raw.compatibility.sustainThresholdDisposition);},"compatibility-declaration-mismatch","/compatibility/sustainThresholdDisposition","legacy-municipal-v1");});
